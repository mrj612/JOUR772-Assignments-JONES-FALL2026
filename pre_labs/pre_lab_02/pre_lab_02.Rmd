...
**Answer So we can just look at the info in that dataset**
...
**Answer: It uses dashes instead of slashes**
...
**Answer They use dashes and all begin with one, showing the entire month instead of specific dates.**
...
**Answer It is creating one column with the total amount of votes and one with just the percentage of votes Wes Moore received.**
...
**Answer I am not able to see the full dataset, but from the code it looks like it arranged the votes from most to least by county.**
...
**Answer For some reason I can see the dataset here, I guess it is useful to for the Moore campaign to see where he is underperforming, although I don't see a blue candidate ever performing well in Garrett County.**
...
**Answer** `str_to_lower(string, locale = "en")`: Converts all characters to lower case.

`str_to_title(string, locale = "en")`: Capitalizes the first letter of each word.
...
**Answer I think it is dividing the data by state**
...
**Answer It does, MD is the second wealthiest state and WV is in the bottom three poorest states.**
...
1.  What percentage of total umd courses are journalism courses?

2.  How many "dance" courses were offered in 2025?

3.  How many journalism courses were offered in the month of January?
